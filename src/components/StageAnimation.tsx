...
export function interpolate(input: number[], output: number[], ease: any = Easing.linear) {
  return (t: number) => {
...
export function animate({ from = 0, to = 1, start = 0, end = 1, ease = Easing.easeInOutCubic }: any) {
  return (t: number) => {
...
export function Sprite({ start = 0, end = Infinity, children, keepMounted = false }: any) {
  ...
  const value: any = { localTime, progress, duration, visible };
...
export function TextSprite({
  ...
  const { localTime, duration } = useSprite() as any;
...
export function ImageSprite({
  ...
  const content = placeholder ? (
    <div style={{
      ...
      {(placeholder as any).label || 'image'}
    </div>
  ) : (
...
export function RectSprite({
  ...
  const spriteCtx = useSprite() as any;
...
export function Stage({
  ...
  React.useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.target && ((e.target as any).tagName === 'INPUT' || (e.target as any).tagName === 'TEXTAREA')) return;
      if (e.code === 'Space') {
        e.preventDefault();
  ...
  return (
    ...
    >
      <div style={{
        ...
        >
          <TimelineContext.Provider value={ctxValue as any}>
            {children}
...
function PlaybackBar({ time, duration, playing, onPlayPause, onReset, onSeek, onHover }: any) {
